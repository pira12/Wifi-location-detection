"""Karma runner — hostapd-mana with enable_karma=1 + dnsmasq + tcpdump."""
...
def hostapd_mana_conf(iface: str, channel: int, karma_out: Path) -> str:
    return (
        f"interface={iface}\n"
        "driver=nl80211\n"
        "ssid=FreeWiFi\n"
...
def main() -> int:
    ...
    pcap = workdir / "karma.pcap"

    hostapd_path.write_text(hostapd_mana_conf(args.ap_iface, args.channel, karma_out))
    ...
    print(f"[*] karma up on {args.ap_iface} ch={args.channel}", flush=True)
